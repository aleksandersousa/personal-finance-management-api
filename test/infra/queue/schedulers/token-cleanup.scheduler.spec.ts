...
import { CRON_PATTERNS } from '@domain/constants';

describe('TokenCleanupScheduler', () => {
  let scheduler: TokenCleanupScheduler;
  let mockQueue: jest.Mocked<Queue<TokenCleanupJobData>>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getQueueToken('token-cleanup'),
          useValue: mockQueue,
        },
      ],
    }).compile();

    scheduler = module.get<TokenCleanupScheduler>(TokenCleanupScheduler);
  ...
  describe('onModuleInit', () => {
    it('should schedule token cleanup job in development environment', async () => {
      // Arrange
      ...
      expect(loggerLogSpy).toHaveBeenCalledWith(
        `Token cleanup recurring job scheduled with pattern: ${CRON_PATTERNS.EVERY_MONTH_AT_2AM}`,
      );
    });

    it('should schedule token cleanup job when IS_WORKER is true', async () => {
      // Arrange
      ...
      expect(loggerLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Token cleanup recurring job scheduled'),
      );
    });
    ...
    it('should handle job already exists error gracefully', async () => {
      ...
      // Assert
      expect(loggerLogSpy).toHaveBeenCalledWith(
        'Token cleanup recurring job already exists',
      );
    ...
    it('should log error for other scheduling failures', async () => {
      ...
      expect(loggerErrorSpy).toHaveBeenCalledWith(
        'Failed to schedule token cleanup job: Queue connection failed',
        'Error stack trace',
      );
    });
